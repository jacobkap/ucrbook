...
table(batch_header_all_years$number_of_months_reported,
      is.na(batch_header_all_years$covered_by_ori),
      useNA = "always")
...
```{r nibrsAnnualPopulation, fig.cap = "The annual population in the United States that is covered by an agency reporting data to NIBRS."}
...
batch_header_all_years %>%
  ...
  ggplot(aes(x = year,
             ...
             color = "12 Months")) +
  geom_line(size = 1.05) +
  geom_line(size = 1.05, aes(y = any_months_population, color = "At Least 1 Month")) +
  xlab("Year") +
  ylab("Population") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma) +
...
```{r nibrsStateParticipation2020, fig.cap = "The percent of each state's population that is covered by police agencies reporting at least one month of data to NIBRS, 2022."}
fips_state_codes <- c(
...
unique(census_state_final$state)
census_state_final <-
  census_state_final %>%
  group_by(state) %>%
...
unique(census_state_final_2000$state)
census_state_final_2000 <-
  census_state_final_2000 %>%
  group_by(state) %>%
...
unique(census_state_final_2010$state)
