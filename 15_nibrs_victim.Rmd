...
kableExtra::kbl(temp, 
             ...
             longtable = TRUE,
             escape = TRUE,
             caption = "The number and percent of crimes committed against each victim. For victims with multiple crimes committed against them, this shows the first crime reported.") %>%
...
kableExtra::kbl(officer_victim_offense_first_year, 
                ...
                caption = "The number and percent of crimes reported for offenses where the victim type was 'law enforcement officer.'") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE, latex_options = c("hold_position", "repeat_header"))

```


```{r nibrsFirstVsAllOffenses, fig.cap="The numbers of murders and sexual offenders when considering only the first offense or all offenses, 1991-2022."}
nibrs_victim_summary  %>%
  ggplot(aes(x = year)) +
  geom_line(aes(y = number_murders_first_offense, color = "Murder, 1st Offense"), size = 1.05) +
  geom_line(aes(y = number_murders_all_offenses , color = "Murder, All Offenses"), size = 1.05) +
  geom_line(aes(y = number_sex_offenses_first_offense, color = "Sex Offense, 1st Offense"), size = 1.05) +
  geom_line(aes(y = number_sex_offenses_all_offenses , color = "Sex Offense, All Offenses"), size = 1.05) +
  xlab("Year") +
  ylab("# of Victims") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma) +
  labs(color = "") +
  expand_limits(y = 0) +
  scale_color_manual(values = c("Murder, 1st Offense" = "#1b9e77",
                                "Murder, All Offenses" = "#d95f02",
                                "Sex Offense, 1st Offense" = "#7570b3",
                                "Sex Offense, All Offenses" = "black"))
```
...
kableExtra::kbl(temp, 
                ...
                escape = TRUE,
                label = "victimType",
                caption = "The distribution of the type of victim. Victim types are mutually exclusive.") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE, latex_options = c("hold_position", "repeat_header"))
```
...
```{r nibrsVictimAssaultInjury, fig.cap="Victim injury for assault offenses, by injury severity, 1991-2022."}
nibrs_victim_summary  %>%
  ggplot(aes(x = year)) +
