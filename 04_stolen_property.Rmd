...
```{r}
ucr_property_stolen_recovered_yearly <- readRDS("data/property_stolen_and_recovered_yearly_1960_2023.rds")
...
```{r phillyHomeBurglaryCount, fig.cap = "The annual number of daytime home burglaries reported in Philadelphia, PA, 1960-2023."}
...
```{r phillyHomeCostPerBurglary, fig.cap = "The annual cost per burglary for daytime home burglaries in Philadelphia, 1960-2023."}
...
temp$inflation_cost_per_burglary[temp$number_of_months_reported < 12] <- NA
ggplot(temp, aes(x = year, y = cost_per_burglary)) +
  geom_path(linewidth = 1.05, aes(color = "Unadjusted")) +
  geom_point(size = 1.05, aes(color = "Unadjusted")) +
  geom_path(linewidth = 1.05, aes(y = inflation_cost_per_burglary, color = "Inflation Adjusted")) +
  geom_point(size = 1.05, aes(y = inflation_cost_per_burglary, color = "Inflation Adjusted")) +
  xlab("Year") +
  ylab("Cost per burglary") +
  theme_crim() +
  scale_y_continuous(labels = scales::dollar) +
  expand_limits(y = 0) +
  labs(color = "") +
  scale_color_manual(values = c("Unadjusted" = "#1b9e77",
                                "Inflation Adjusted" = "#d95f02"))
...
ucr_property_stolen_recovered_yearly %>%
  ...
  geom_line(data = property_any_month_reporting, linewidth = 1.05, aes(y = n, color = "At Least 1 Month")) +
  xlab("Year") +
...
```{r agenciesInBoth, fig.cap = "The percent of agencies in the Supplement to Return A data that report at least one month of data, and all 12 months of data, and are also in the Offenses Known and Clearances by Arrest (Return A) data in that year, 1960-2023."}
...
agencies_in_offenses_12 <- data.frame(year = 1960:2022,
                                   offenses_agencies = NA,
                                   property_agencies = NA,
                                   percent = NA)
for (i in 1:nrow(agencies_in_offenses_12)) {
  year_temp <- agencies_in_offenses_12$year[i]
  offenses_agencies <- offenses_known_yearly %>%
    dplyr::filter(year %in% year_temp,
  ...
  agencies_in_offenses_12$offenses_agencies[i] <- nrow(offenses_agencies) 
  agencies_in_offenses_12$property_agencies[i] <- nrow(property_agencies) 
  agencies_in_offenses_12$percent[i] <- nrow(property_agencies) / 
    nrow(offenses_agencies) 
  
}

ggplot(agencies_in_offenses, aes(x = year, y = percent)) +
  geom_line(linewidth = 1.05, aes(color = "At Least One Month")) +
  geom_line(data = agencies_in_offenses_12, linewidth = 1.05, aes(color = "12 Months")) +
  xlab("Year") +
  ylab("% of Agencies ") +
  theme_crim() +
  scale_y_continuous(labels = scales::percent, expand = c(0, 0)) +
  labs(color = "Months Reported") +
  scale_color_manual(values = c("12 Months" = "#1b9e77",
                                "At Least One Month" = "#d95f02"))
...
```{r theftByValue, fig.cap = "The annual breakdown in total theft value by the three value categories: less than \\$50, \\$50-\\$199, and \\$200 and over, among all agencies that reported 12 months of data in that year, 1960-2023."}
ucr_property_stolen_recovered_yearly %>%
  ...
  ggplot(aes(x = year,
             ...
             color = "<$50")) +
  geom_line(linewidth = 1.05) +
  geom_line(aes(y = offenses_theft_50_to_200, color = "$50-$200"), linewidth = 1.05) +
  geom_line(aes(y = offenses_theft_over_200, color = ">$200"), linewidth = 1.05) +
  xlab("Year") +
  ylab("% of Thefts") +
...
```{r nycLivestock, fig.cap = "The annual value of stolen livestock in New York City, 1960-2023."}
ucr_property_stolen_recovered_yearly %>%
...
```{r romeConsumable, fig.cap = "The annual value of stolen consumable goods, bicycles, livestock, and thefts from pickpocketing, in Rome, New York, 1960-2023."}
ucr_property_stolen_recovered_yearly %>%
  dplyr::filter(ori %in% "NY03201") %>%
  ggplot(aes(x = year, y = value_stolen_consumable_good)) +
  geom_line(linewidth = 1.05, aes(color = "Consumable Goods")) +
  geom_line(linewidth = 1.05, aes(y = value_theft_bicycle, color = "Bicycle")) +
  geom_line(linewidth = 1.05, aes(y = value_stolen_livestock, color = "Livestock")) +
  geom_line(linewidth = 1.05, aes(y = value_theft_pick_pocket, color = "Pickpocket")) +
  geom_point(size = 1.05, aes(color = "Consumable Goods")) +
  geom_point(size = 1.05, aes(y = value_theft_bicycle, color = "Bicycle")) +
  geom_point(size = 1.05, aes(y = value_stolen_livestock, color = "Livestock")) +
  geom_point(size = 1.05, aes(y = value_theft_pick_pocket, color = "Pickpocket")) +
  xlab("Year") +
  ylab("Value of property stolen") +
  theme_crim() +
  scale_y_continuous(labels = scales::dollar) +
  ...
  scale_color_manual(values = c("Consumable Goods" = "#1b9e77",
                                "Bicycle" = "#d95f02",
                                "Livestock" = "#7570b3",
                                "Pickpocket" = "black"))
```
