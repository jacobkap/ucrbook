...
leoka <- readRDS("data/leoka_yearly_1960_2022.rds")
```
...
```{r leokaNationalEmployees, fig.cap = "The number of civilian employees and sworn officers in the United States, 1960-2022"}
leoka %>%
...
```{r leokaNationalOfficers, fig.cap = "The percent of sworn officers by gender in the United States, 1960-2022"}
temp <- leoka %>%
  distinct(year, ori, .keep_all = TRUE) %>%
  group_by(year) %>%
...
```{r leokaCivilianOfficers, fig.cap = "The number of civilian employees and sworn officers in the Philadelphia Police Department, 1960-2022"}
...
```{r leokaOfficersGender, fig.cap = "The percent of female and male sworn officers in Philadelphia, 1960-2022"}
...
```{r leokaOfficersKilled, fig.cap = "The number of officers killed by felony and killed accidentally in New York City, 1960-2022"}
leoka %>%
...
```{r leokaOfficersKilledNational, fig.cap = "The national number of officers killed by felony and killed accidentally, 1960-2022"}
leoka %>%
  group_by(year) %>%
...
```{r leokaAssaultTypeInjury, fig.cap = "The total number of assaults on officers by injury sustained and offender weapon in Los Angeles, 1960-2022.", fig.height = 18}
assault_type <- leoka %>%
...
```{r leokaAssaultsInjuryYear, fig.cap = "The annual number of assaults on officers by injury sustained in New York City, 1960-2022."}
leoka %>%
...
```{r leokaAssaultCallType, fig.cap = "Assaults on Los Angeles Police Department officers by type of call where they were assaulted at, 1960-2022."}
call_type <- leoka %>%
  filter(ori %in% "CA01942") %>%
  mutate(dummy = 1) %>%
  select(dummy,
...
```{r leokaDisturbanceWeapon, fig.cap = "The number of assaults on Los Angeles Police Department officers in disturbance calls by the injury sustained by the officer, 1960-2022."}
call_type <- leoka %>%
  filter(ori %in% "CA01942") %>%
  mutate(dummy = 1) %>%
  select(dummy,
...
```{r leokaShiftAssignment, fig.cap = "The number of assaults on Los Angeles Police Department officers in disturbance calls by the injury sustained by the shift assignment of the officer, 1960-2022."}
call_type <- leoka %>%
  filter(ori %in% "CA01942") %>%
  mutate(dummy = 1) %>%
  select(dummy,
