...
```{r leokaOfficersGenderCount, fig.cap = "The number of female and male sworn officers in Philadelphia, 1960-2022"}
leoka %>%
  dplyr::filter(ori %in% "PAPEP00") %>%
  ggplot(aes(x = year, y = female_employees_officers, color = "Female Officers")) +
  geom_line(size = 1.05) +
...
```{r leokaOfficersKilled, fig.cap = "The number of officers killed by felony and killed accidentally in Los Angeles, 1960-2022"}
leoka %>%
...
```{r leokaAssaultTypeInjury, fig.cap = "The total number of assaults on officers by injury sustained and offender weapon in Los Angeles, 1960-2022.", fig.height=15}
assault_type <- leoka %>%
...
```{r leokaAssaultsInjuryYear, fig.cap = "The annual number of assaults on officers by injury sustained in Los Angeles, 1960-2022."}
leoka %>%
  filter(ori %in% "CA01942") %>%
