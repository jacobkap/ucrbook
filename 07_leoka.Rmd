...
leoka <- readRDS("data/leoka_yearly_1960_2023.rds")
```
...
```{r leokaAgencies, fig.cap = "The annual number of police agencies that report at least month of data, at least one employee, and at least one assault against an officer, 1960-2023"}
...
leoka %>%
  ...
  ggplot(aes(x = year, y = n, color = "\u2265 One Employee")) +
  geom_line(linewidth = 1.05) + 
  geom_line(linewidth = 1.05, aes(y = at_least_one_month, color = "\u2265 One Month")) +
  geom_line(linewidth = 1.05, aes(y = at_least_one_assault, color = "\u2265 One Assault")) +
  xlab("Year") +
  ylab("# of Agencies") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
  expand_limits(y = 0) +
  scale_color_manual(values = c("\u2265 One Employee" = "#1b9e77",
                                "\u2265 One Month" = "#d95f02",
                                "\u2265 One Assault" = "#7570b3")) +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
  labs(color = "")
...
```{r leokaNationalEmployees, fig.cap = "The number of civilian employees and sworn officers in the United States, 1960-2023"}
leoka %>%
  distinct(year, ori, .keep_all = TRUE) %>%
  group_by(year) %>%
  summarize(total_employees_officers = sum(total_employees_officers, na.rm = TRUE),
            total_employees_civilians = sum(total_employees_civilians, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(aes(x = year, y = total_employees_officers, color = "Officers")) +
  geom_line(linewidth = 1.05) +
  geom_line(aes(y = total_employees_civilians, color = "Civilians"), linewidth = 1.05) +
  xlab("Year") +
  ylab("# of Employees") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
...
```{r leokaNationalOfficers, fig.cap = "The percent of sworn officers by gender in the United States, 1960-2023"}
temp <- leoka %>%
  distinct(year, ori, .keep_all = TRUE) %>%
  group_by(year) %>%
  summarize(male_employees_officers = sum(male_employees_officers, na.rm = TRUE),
            female_employees_officers = sum(female_employees_officers, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(total_officers = male_employees_officers + female_employees_officers,
         male_prop = male_employees_officers / total_officers,
         female_prop = female_employees_officers / total_officers) %>%
  ggplot(aes(x = year, y = male_prop, color = "Male Officers")) +
  geom_line(linewidth = 1.05) +
  geom_line(aes(y = female_prop, color = "Female Officers"), linewidth = 1.05) +
  xlab("Year") +
  ylab("# of Employees") +
  theme_crim() +
  ...
  expand_limits(y = 0) +
  labs(color = "") +
  scale_color_manual(values = c("Female Officers" = "#1b9e77",
                                "Male Officers" = "#d95f02"))
```
...
```{r leokaCivilianOfficers, fig.cap = "The number of civilian employees and sworn officers in the Philadelphia Police Department, 1960-2023"}
leoka$total_employees_officers[leoka$ori %in% "PAPEP00" &
                                 leoka$total_employees_officers %in% 0] <- NA
leoka$total_employees_civilians[leoka$ori %in% "PAPEP00" &
                                  leoka$total_employees_civilians %in% 0] <- NA
leoka %>%
  dplyr::filter(ori %in% "PAPEP00") %>%
  ggplot(aes(x = year, y = total_employees_officers, color = "Officers")) +
  geom_path(linewidth = 1.05) +
  geom_path(aes(y = total_employees_civilians, color = "Civilians"), linewidth = 1.05) +
  geom_point(size = 1.05, aes(y = total_employees_officers, color = "Officers")) +
  geom_point(aes(y = total_employees_civilians, color = "Civilians"), size = 1.05) +
  xlab("Year") +
  ylab("# of Employees") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
  expand_limits(y = 0) +
  labs(color = "Employee Type") +
  scale_color_manual(values = c("Officers" = "#1b9e77",
                                "Civilians" = "#d95f02"))
```
...
```{r leokaOfficersGender, fig.cap = "The percent of female and male sworn officers in Philadelphia, 1960-2023"}
...
temp$male_employees_officers[temp$total_officers %in% 0] <- NA
temp$female_employees_officers[temp$total_officers %in% 0] <- NA
temp %>%
  ggplot(aes(x = year, y = female_employees_officers, color = "Female Officers")) +
  geom_path(linewidth = 1.05) +
  geom_point(size = 1.05) +
  geom_path(aes(y = male_employees_officers, color = "Male Officers"), linewidth = 1.05) +
  geom_point(aes(y = male_employees_officers, color = "Male Officers"), size = 1.05) +
  xlab("Year") +
  ylab("% of Officers") +
  theme_crim() +
  scale_y_continuous(labels = scales::percent, expand = c(0, 0), limits = c(0, NA)) +
...
```{r leokaOfficersKilled, fig.cap = "The number of officers killed by felony and killed accidentally in New York City, 1960-2023"}
leoka %>%
  dplyr::filter(ori %in% "NY03030") %>%
  ggplot(aes(x = year, y = officers_killed_by_felony, color = "Killed by Felony")) +
  geom_line(linewidth = 1.05) +
  geom_line(aes(y = officers_killed_by_accident, color = "Killed Accidentally"), linewidth = 1.05) +
  xlab("Year") +
  ylab("# of Officers") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
...
```{r leokaOfficersKilledNational, fig.cap = "The national number of officers killed by felony and killed accidentally, 1960-2023"}
leoka %>%
  group_by(year) %>%
...
```{r leokaAssaultTypeInjury, fig.cap = "The total number of assaults on officers by injury sustained and offender weapon in Los Angeles, 1960-2023.", fig.height = 18}
assault_type <- leoka %>%
...
```{r leokaAssaultsInjuryYear, fig.cap = "The annual number of assaults on officers by injury sustained in New York City, 1960-2023."}
leoka %>%
  filter(ori %in% "NY03030") %>%
  ...
  geom_line(linewidth = 1.05) +
  geom_line(aes(y = assaults_with_injury_total, color = "With Injuries"), linewidth = 1.05) +
  geom_line(aes(y = assaults_no_injury_total, color = "No Injuries"), linewidth = 1.05) +
  xlab("Year") +
  ylab("# of Assaults") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
...
```{r leokaAssaultCallType, fig.cap = "Assaults on Los Angeles Police Department officers by type of call where they were assaulted at, 1960-2023."}
call_type <- leoka %>%
  filter(ori %in% "CA01942") %>%
  mutate(dummy = 1) %>%
  select(dummy,
...
```{r leokaDisturbanceWeapon, fig.cap = "The number of assaults on Los Angeles Police Department officers in disturbance calls by the injury sustained by the officer, 1960-2023."}
call_type <- leoka %>%
  filter(ori %in% "CA01942") %>%
  mutate(dummy = 1) %>%
  select(dummy,
...
call_type$type <- gsub("Assaults?", "", call_type$type)
call_type$type <- trimws(call_type$type)
...
```{r leokaShiftAssignment, fig.cap = "The number of assaults on Los Angeles Police Department officers in disturbance calls by the injury sustained by the shift assignment of the officer, 1960-2023."}
call_type <- leoka %>%
  filter(ori %in% "CA01942") %>%
  mutate(dummy = 1) %>%
  select(dummy,
