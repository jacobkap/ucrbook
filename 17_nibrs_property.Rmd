...
```{r nibrsPropertyAgenciesReporting, fig.cap="The annual number of agencies reporting data in the NIBRS Property Segment, 1991-2022."}
...
kableExtra::kbl(temp, 
             ...
             escape = TRUE,
             caption = "The number and percent of property stolen (including forcibly taken such as during a robbery) in a crime, for all offenses in 2022. Each incident can have multiple items stolen.") %>%
...
temp <- make_frequency_table(property[property$type_of_property_loss %in% "seized", ],
                             "property_description",
                             c("Property",
                               "# of Property Seized",
                               "% of Property Seized"))
kableExtra::kbl(temp, 
          #   format = "html",
             ...
             #booktabs = TRUE, 
             longtable = TRUE,
             label = "propertySeizedDescription",
             escape = TRUE,
             caption = "The number and percent of property seized by police (excludes recovering property that was stolen, for all offenses in 2022. Each incident can have multiple items seized.") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE, latex_options = c("hold_position", "repeat_header"))
```
...
```{r propertyDaysUntilRecovered, fig.cap = "The distribution of the number of days from the incident to the property recovered date. In 2022 the maximum days from incident to arrest was 456 days. Zero days means that the arrest occurred on the same day as the incident. "}
...
```{r nibrsPropertySeizures, fig.cap="The annual number of drug seizures reported, 1991-2022."}
nibrs_property_summary  %>%
  ungroup() %>%
  ggplot(aes(x = year, y = number_of_seizures)) +
  geom_line(size = 1.05) +
  xlab("Year") +
  ylab("# of Seizures") +
  theme_crim() +
...
nibrs_property_summary  %>%
  ggplot(aes(x = year)) +
  geom_line(aes(y = percent_marijuana_hashish, color = "Marijuana/Hashish"), size = 1.05) +
  geom_line(aes(y = percent_cocaine_crack, color = "Cocaine/Crack"), size = 1.05) +
  geom_line(aes(y = percent_meth , color = "Methamphetamine"), size = 1.05) +
  geom_line(aes(y = percent_heroin_opium , color = "Heroin/Opium"), size = 1.05) +
  geom_line(aes(y = percent_other_drug , color = "Other Drug"), size = 1.05) +
  geom_line(aes(y = percent_unknown_drug , color = "Unknown Drug"), size = 1.05) +
  xlab("Year") +
  ylab("% of Incidents") +
  theme_crim() +
  scale_y_continuous(labels = scales::percent) +
