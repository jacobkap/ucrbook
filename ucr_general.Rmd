...
```{r sf_thefts, fig.cap = "Thefts and total index crimes in San Francisco, 1960-2018." }
sf <- offenses_known_yearly_1960_2018[offenses_known_yearly_1960_2018$ori %in% "CA03801", ]

ggplot(sf, aes(x = year, y = actual_index_total, color = "Total Index Crimes")) +
  geom_line(size = 1.05) +
  geom_line(aes(y = actual_theft_total, color = "Theft"), size = 1.05) +
  geom_line(aes(y = actual_index_violent, color = "Violent Index Crimes"), size = 1.05) +
  xlab("Year") +
...
```{r chicago_murder, fig.cap = "Murders in Chicago as a percent of violent index crimes, 1960-2018."}
chicago <- offenses_known_yearly_1960_2018[offenses_known_yearly_1960_2018$ori %in% "ILCPD00", ]
...
```{r simple_vs_index, fig.cap="Reported crimes in Philadelphia from 1960 to 2018. Violent index crimes are aggravated assault, rape, robbery, and murder."}
houston <- offenses_known_yearly_1960_2018[offenses_known_yearly_1960_2018$ori %in% "TXHPD00",]

ggplot(houston, aes(x = year, y = actual_index_violent, color = "Violent Index Crimes")) +
  geom_line(size = 1.05) +
  geom_line(aes(y = actual_assault_simple, color = "Simple Assault"), size = 1.05) +
  geom_line(aes(y = actual_assault_aggravated, color = "Aggravated Assault"), size = 1.05) +
  xlab("Year") +
  ylab("Crimes Reported") +
  labs(color = "Crime Category") +
  theme_crim() +
...
```{r agencies_reporting, fig.cap="The annual number of agencies reporting to the Offenses Known and Clearances by Arrest dataset. Reporting is based on the agency reporting at least one month of data in that year."}
...
```{r nyc_gun_assaults, fig.cap = "Monthly reports of gun assaults in New York City, 1960-2018."}
nyc_annual <- offenses_known_yearly_1960_2018[offenses_known_yearly_1960_2018$ori %in% "NY03030",]
ggplot(nyc, aes(x = year, y = actual_assault_with_a_gun)) +
  geom_line(size = 1.05) +
  xlab("Year") +
  ylab("Gun Assaults") +
  theme_crim() +
...
```{r nyc_murder_monthly, fig.cap = "Monthly murders in New York City, 1960-2018. During the 2000s, the police department began reporting quarterly instead of monthly and then resumed monthly reporting."}
ggplot(nyc, aes(x = year, y = actual_murder)) +
  geom_line(size = 1.05) +
  xlab("Year") +
  ylab("Murders") +
  theme_crim()
...
```{r nyc_murder_yearly, fig.cap = "Annual murders in New York City, 1960-2018."}

ggplot(nyc_annual, aes(x = year, y = actual_murder)) +
  geom_line(size = 1.05) +
  xlab("Year") +
  ylab("Murders") +
  theme_crim()
