...
nibrs_offense <- readRDS("data/nibrs_offense_segment_2023.rds")
```
...
kableExtra::kbl(hierarchy_table,  
                #  format = "latex",
                digits = 2, 
                align = c("l", "l", "r", "r"),
                booktabs = TRUE, 
...
```{r sfThefts, fig.cap = "Thefts and total index crimes in San Francisco, 2000-2023." }
sf <- offenses_known_yearly[offenses_known_yearly$ori %in% "CA03801", ]
sf$actual_theft_total[sf$number_of_months_reported %in% 0] <- NA
sf$actual_index_violent[sf$number_of_months_reported %in% 0] <- NA
sf$actual_index_total[sf$number_of_months_reported %in% 0] <- NA
sf %>%
  filter(year >= 2000) %>%
  ggplot(aes(x = year, y = actual_index_total, color = "Total Index")) +
  geom_path(linewidth = 1.05) +
  geom_path(aes(y = actual_theft_total, color = "Theft"), linewidth = 1.05) +
  geom_path(aes(y = actual_index_violent, color = "Violent Index"), linewidth = 1.05) +
  geom_point(size = 1.05) +
  geom_point(aes(y = actual_theft_total, color = "Theft"), geom_point = 1.05) +
  geom_point(aes(y = actual_index_violent, color = "Violent Index"), geom_point = 1.05) +
  xlab("Year") +
  ylab("Crimes Reported") +
  labs(color = "Crime") +
  theme_crim() +
  theme(legend.position = "bottom") +
  scale_color_manual(values = c("Total Index" = "#1b9e77", "Theft" = "#d95f02", "Violent Index" ="#7570b3")) +
...
```{r chicagoMurder, fig.cap = "Murders in Chicago as a percent of violent index crimes, 1960-2023."}
chicago <- offenses_known_yearly[offenses_known_yearly$ori %in% "ILCPD00", ]
chicago$murder_percent_of_index <- chicago$actual_murder / chicago$actual_index_violent

ggplot(chicago, aes(x = year, y = murder_percent_of_index)) +
  geom_line(linewidth = 1.05) +
  xlab("Year") +
  ylab("% of violent index crimes") +
  theme_crim() +
...
```{r simpleIndex, fig.cap="Reported crimes in Houston, Texas, from 1960 to 2018. Violent index crimes are aggravated assault, rape, robbery, and murder."}
houston <- offenses_known_yearly[offenses_known_yearly$ori %in% "TXHPD00",]

ggplot(houston, aes(x = year, y = actual_index_violent, color = "Violent Index")) +
  geom_line(linewidth = 1.05) +
  geom_line(aes(y = actual_assault_simple, color = "Simple Assault"), linewidth = 1.05) +
  geom_line(aes(y = actual_assault_aggravated, color = "Aggravated Assault"), linewidth = 1.05) +
  xlab("Year") +
  ylab("Crimes Reported") +
  labs(color = "") +
  theme_crim() +
  theme(legend.position = "bottom") +
  scale_color_manual(values = c("Violent Index" = "#1b9e77", "Simple Assault" = "#d95f02", "Aggravated Assault" = "#7570b3")) +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA))
```
...
```{r offensesAgenciesReporting, fig.cap="The annual number of agencies reporting at least one month of data and a full 12 months of data, 1960-2023."}
...
offenses_known_yearly %>%
  ...
  geom_line(data = offense_12_months, aes(y = last_month_full, color = "Last\n Month\n Reported;\n 12 months"), linewidth = 1.05) +
  geom_line(data = offense_12_months, aes(y = months_missing_full, color = "Months\n Missing;\n 12 months"), linewidth = 1.05) +
  xlab("Year") +
  ylab("# of Agencies") +
  theme_crim() +
  scale_color_manual(values = c("Last\n Month\n Reported;\n >= 1 month" = "#1b9e77",
                                "Months\n Missing;\n >=1 month" = "#d95f02",
                                "Last\n Month\n Reported;\n 12 months" = "#7570b3",
                                "Months\n Missing;\n 12 months" = "black")) +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
...
```{r murdersBothMeasures, fig.cap = "The percent change in the number of murders reported in the United States each year when moving from the number of months missing measure to the last month reported measure, 1960-2020."}
last_month_murder <- 
  offenses_known_yearly %>%
...
```{r newarkMurders, fig.cap = "The annual murder rate per 100,000 people in Newark, NJ, 1960-2023."}
offenses_known_yearly %>%
...
```{r lapdClearance, fig.cap = "The annual number of actual and cleared murders from the Los Angeles Police Department, 1960-2023."}
offenses_known_yearly$actual_murder[offenses_known_yearly$ori %in% "CA01942" &
                                      offenses_known_yearly$number_of_months_missing %in% 12] <- NA
offenses_known_yearly$tot_clr_murder[offenses_known_yearly$ori %in% "CA01942" &
                                       offenses_known_yearly$number_of_months_missing %in% 12] <- NA
offenses_known_yearly %>%
  filter(ori %in% "CA01942") %>%
  ggplot(aes(x = year, y = actual_murder)) +
  geom_path(aes(color = "Actual Murders"), linewidth = 1.05) +
  geom_path(aes(y = tot_clr_murder, color = "Cleared Murders"), linewidth = 1.05) +
  geom_point(aes(color = "Actual Murders"), linewidth = 1.05) +
  geom_point(aes(y = tot_clr_murder, color = "Cleared Murders"), linewidth = 1.05) +
  xlab("Year") +
  ylab("# of Crimes") +
  theme_crim() +
...
```{r frankenmuthRape, fig.cap = "The number of actual burglaries reported by the Frankenmuth Police Department, MI, 1960-2023."}
offenses_known_yearly %>%
...
```{r denverUnfounded, fig.cap = "The annual number of unfounded crimes in Denver, CO, 1983-2023."}
offenses_known_yearly %>%
...
```{r rapePhilly, fig.cap = "The annual number of rapes reported in Philadelphia, San Francisco, New York City, and Jackson, MS, 2005-2019."}
philly_rape <-
  offenses_known_yearly %>%
  dplyr::filter(ori %in% "PAPEP00",
                year %in% 2000:2019) %>%
  mutate(rape_rate = actual_rape_total / population * 100000) %>%
  select(year,
         number_of_months_missing,
         rape_rate) %>%
  mutate(agency = "Philadelphia")
...
jackson_rape <-
  ...
  select(year,
         number_of_months_missing,
         rape_rate) %>%
  mutate(agency = "Jackson, MS")

philly_rape$rape_rate[philly_rape$number_of_months_missing > 0] <- NA
nyc_rape$rape_rate[nyc_rape$number_of_months_missing > 0] <- NA
sf_rape$rape_rate[sf_rape$number_of_months_missing > 0] <- NA
jackson_rape$rape_rate[jackson_rape$number_of_months_missing > 0] <- NA

philly_rape %>%
  ggplot(aes(x = year, y = rape_rate)) +
  geom_path(linewidth = 1.05, aes(color = "Philadelphia")) +
  geom_point(size = 1.05, aes(color = "Philadelphia")) +
  geom_path(data = nyc_rape, linewidth = 1.05, aes(color = "New York City")) +
  geom_point(data = nyc_rape, size = 1.05, aes(color = "New York City")) +
  geom_path(data = sf_rape, linewidth = 1.05, aes(color = "San Francisco")) +
  geom_point(data = sf_rape, size = 1.05, aes(color = "San Francisco")) +
  geom_path(data = jackson_rape, linewidth = 1.05, aes(color = "Jackson, MS")) +
  geom_point(data = jackson_rape, size = 1.05, aes(color = "Jackson, MS")) +
  xlab("Year") +
  ylab("Rapes per 100k Population") +
  theme_crim() +
  scale_y_continuous(labels = scales::comma, expand = c(0, 0), limits = c(0, NA)) +
  expand_limits(y = 0) +
  labs(color = "") +
  scale_color_manual(values = c("Philadelphia" = "#1b9e77",
                                "New York City" = "#d95f02",
                                "San Francisco" = "#7570b3",
                                "Jackson, MS" = "black")) +
  scale_x_continuous(breaks = c(2000, 2003, 2006, 2009, 2012, 2013, 2014, 2016, 2019))
...
```{r manslaughterVsMurder, fig.cap = "The annual number of murder and non-negligent manslaughter, manslaughter by negligence, and the sum of the two, nationwide from 1960-2023."}
offenses_known_yearly %>%
  group_by(year) %>%
...
```{r manslaughterPercent, fig.cap = "Reported manslaughter by negligence as a percent of reported murder and non-negligent manslaughter, nationwide 1960-2023."}
offenses_known_yearly %>%
  group_by(year) %>%
  summarize(murder = sum(actual_murder),
            manslaughter = sum(actual_manslaughter)) %>%
