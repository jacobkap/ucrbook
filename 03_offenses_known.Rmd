...
sf$actual_theft_total[sf$number_of_months_missing %in% 12] <- NA
sf$actual_index_violent[sf$number_of_months_missing %in% 12] <- NA
sf$actual_index_total[sf$number_of_months_missing %in% 12] <- NA
sf %>%
  filter(year >= 2000) %>%
...
offenses_known_yearly %>%
  ...
  geom_line(data = offense_12_months, aes(y = last_month_full, color = "Last Month\n Reported;\n 12 months"), linewidth = 1.05) +
