...
```{r offensesbigAgenciesReporting, fig.cap = "The annual number of agencies with a population of 100,000 or higher reporting to the Offenses Known and Clearances by Arrest dataset. Reporting is based on the agency reporting at least one month of data in that year."}
offenses_known_yearly %>%
...
```{r newarkMurders, fig.cap = "The annual number of murders in Newark, NJ, 1960-2022."}
offenses_known_yearly %>%
...
```{r phillyRapeUnfound, fig.cap = "The annual number of actual and unfounded rapes in Philadelphia, PA, 1960-2022."}
offenses_known_yearly %>%
...
```{r phillyRapeUnfoundPercent, fig.cap = "The percent of reported rapes that the police recorded as unfounded in Philadelphia, PA, and the United States, 1984-2022."}
national_unfounded_rape <-
  offenses_known_yearly %>%
  group_by(year) %>%
  summarize(unfound_rape_total = sum(unfound_rape_total, na.rm = TRUE),
            actual_rape_total = sum(actual_rape_total, na.rm = TRUE)) %>%
  ungroup() %>%
  dplyr::mutate(rape_unfounded_national = unfound_rape_total / (actual_rape_total+unfound_rape_total))  %>%
  select(year,
         rape_unfounded_national)

offenses_known_yearly %>%
  dplyr::filter(ori %in% "PAPEP00",
                year > 1983) %>%
  dplyr::mutate(rape_unfounded = unfound_rape_total / (actual_rape_total+unfound_rape_total)) %>%
  left_join(national_unfounded_rape) %>%
  ggplot(aes(x = year, y = rape_unfounded)) +
  geom_line(aes(color = "Philly"), size = 1.02) +
  geom_line(aes(y = rape_unfounded_national, color = "National"), size = 1.02) +
  xlab("Year") +
  ylab("% of Rapes Unfounded") +
  theme_crim() +
  scale_y_continuous(labels = scales::percent) +
  labs(color = "") +
  scale_color_manual(values = c("Philly" = "#1b9e77",
                                "National" = "#d95f02")) 
```
...
```{r phillyRobberyUnfoundPercent, fig.cap = "The percent of reported robberies that the police recorded as unfounded in Philadelphia, PA, and the United States, 1984-2022."}
national_unfounded_robbery <-
  offenses_known_yearly %>%
  group_by(year) %>%
  summarize(unfound_robbery_total = sum(unfound_robbery_total, na.rm = TRUE),
            actual_robbery_total = sum(actual_robbery_total, na.rm = TRUE)) %>%
  ungroup() %>%
  dplyr::mutate(robbery_unfounded_national = unfound_robbery_total / (actual_robbery_total+unfound_robbery_total))  %>%
  select(year,
         robbery_unfounded_national)
  
offenses_known_yearly %>%
  dplyr::filter(ori %in% "PAPEP00",
                year > 1983) %>%
  dplyr::mutate(robbery_unfounded = unfound_robbery_total / (actual_robbery_total+unfound_robbery_total)) %>%
  left_join(national_unfounded_robbery) %>%
  ggplot(aes(x = year, y = robbery_unfounded)) +
  geom_line(aes(color = "Philly"), size = 1.02) +
  geom_line(aes(y = robbery_unfounded_national, color = "National"), size = 1.02) +
  xlab("Year") +
  ylab("% of robberys Unfounded") +
  theme_crim() +
  crimeutils::scale_color_crim() + 
  scale_y_continuous(labels = scales::percent) +
  labs(color = "") +
