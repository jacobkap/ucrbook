...
load("data/ucr_arrests_yearly_all_crimes_race_1974_1994.rda")
load("data/ucr_arrests_yearly_all_crimes_race_1995_2019.rda")


all_crimes <- ucr_arrests_yearly_all_crimes_race_1974_1994 %>%
  bind_rows(ucr_arrests_yearly_all_crimes_race_1995_2019)
rm(ucr_arrests_yearly_all_crimes_race_1974_1994)
rm(ucr_arrests_yearly_all_crimes_race_1995_2019)
invisible(gc())


load("data/ucr_arrests_yearly_index_crimes_age_1974_1994.rda")
load("data/ucr_arrests_yearly_index_crimes_age_1995_2019.rda")
index <- ucr_arrests_yearly_index_crimes_age_1974_1994 %>%
  bind_rows(ucr_arrests_yearly_index_crimes_age_1995_2019)
rm(ucr_arrests_yearly_index_crimes_age_1974_1994)
rm(ucr_arrests_yearly_index_crimes_age_1995_2019)
invisible(gc())
```
...
Arrest data is available from 1974 through 2019, but as arrest reporting is voluntary, there is substantial variation in the number of agencies reporting in a given month.  Figure \@ref(fig:arrestsAgenciesReporting) shows how many agencies reported at least one arrest in one month of the year for each of these years. The first year of data has about 9,000 agencies reporting and that increases strongly to a little over 13,000 in 1979. Following the odd blip in 1980, the number of agencies remain steady for nearly a decade before declining to a local low of about 11,000 in 1998 before again increasing steadily until the end of our data where nearly 15,000 agencies report. This 15,000, however, still remains under the estimated 18,000 police agencies in the United States and below the reporting rates of UCR data such as the Offenses Known and Clearances by Arrest data. This data is also missing some important cities such as New York City which hasn't reported even a single month since 2002 and Chicago which tends to only report a single month if at all. (I'm not sure why there's a dip in 1980. Since it immediately reverses itself in the next year I think it's just a data issue, not a real decrease in the number of agencies that report.)

```{r arrestsAgenciesReporting, fig.cap="The annual number of agencies reporting at least one month of data in that year."}
all_crimes %>%
...
```{r arrestsAgenciesReporting12Months, fig.cap="The annual number of agencies reporting 12 months of data in that year."}
all_crimes %>%
...
To aid in analysis of reporting patterns, in my openICPSR data users will find a set of constructed variables that summarize *apparent* reporting behaviors (these are the variables used to create the figures above). These are included because the FBI does not provide any explicit data on when they have received reports from an agency. 

`number_of_months_reported`, for example, reports the number of months within the calender year in which a given agency reported *at least one arrest*. Similarly, the dataset also includes a set of variables of the format `num_months_[crime]` that report the number of months within the calender year in which a given agency has reported *at least one arrest*. However, it is important to emphasize two limitations of these metrics:

- UCR data provided by the FBI does not differentiate between "no arrests" and "nothing reported" (see [Section 2.2.4](https://ucrbook.com/ucrGeneral.html#zero-crimes-vs-no-reports)), so `number_of_months_reported` may underestimate actual months reported if an agency just has a month with no arrests (since it just counts months with any arrests accounted). Obviously this is unlikely to be a concern for bigger agencies, but it may be an issue for smaller agencies. 
- Agencies have the option of only reporting arrests for specific crimes. Thus it is always possible that an agency that reports some crimes in every month of the year may *not* be reporting arrests for all categies of crime in every month. So if you're interested in a particular category of crime, make sure to inspect the `num_months_[crime]` in the data (remembering the concern above). 

**Nota bene:** the `number_of_months_reported` variable in my openICPSR data released prior to data Version 13 sometimes under-estimated the number of months with reported arrests, so be sure to use the most up-to-date version of the openICPSR data to ensure your analysis reflects the collective wisdom of all users.
...
```{r phillyRapeAge, fig.cap = "The total number of rapes by male arrestees reported by arrestee age in Philadelphia, 1974-2019.", fig.height=14}
...
```{r phillyMarijuanaRacePercent, fig.cap = "The annual percent of adult marijuana possession arrests in Philadelphia by arrestee race, 1976-2019."}
...
```{r phillyMarijuanaRaceCount, fig.cap = "The annual number of adult marijuana possession arrests in Philadelphia by arrestee race, 1976-2019."}
...
```{r phillyJuvenileReferrals, fig.cap = "The annual number of juvenile referrals in Philadelphia by referral type, 1974-2019."}

all_crimes %>%
  filter(ori %in% "PAPEP00") %>%
  ggplot(aes(x = year)) +
  geom_line(aes(y = juv_handled_within_department,
                color = "Internally"), size = 1.05) +
  geom_line(aes(y = juv_referred_to_juv_court,
                color = "Juvenile court"), size = 1.05) +
  geom_line(aes(y = juv_referred_to_welfare,
                color = "Welfare"), size = 1.05) +
  geom_line(aes(y = juv_referred_to_police,
                color = "Other police"), size = 1.05) +
    geom_line(aes(y = juv_referred_to_crim_court,
                color = "Adult court"), size = 1.05) +
  ylab("# of Arrests") +
  xlab("Year") +
  labs(color = "") +
  theme_crim() +
