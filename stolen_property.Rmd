...
```{r}
load("data/ucr_property_stolen_recovered_yearly_1960_2019.rda")
load("data/offenses_known_yearly_1960_2018.rda")


value_columns <- grep("value|offenses", names(ucr_property_stolen_recovered_yearly_1960_2019), value = TRUE)
final <- data.frame(variable = value_columns,
                    first_year_reported = NA)

grouped_by_year <- ucr_property_stolen_recovered_yearly_1960_2019 %>%
    group_by(year) %>%
...
```{r phillyHomeBurglaryCount, fig.cap ="The annual number of daytime home burglaries reported in Philadelphia, PA, 1960-2019."}
temp <- ucr_property_stolen_recovered_yearly_1960_2019 %>%
...
```{r phillyHomeCostPerBurglary, fig.cap = "The annual number of burglaries and cost per burglary for daytime home burglaries in Philadelphia, 1960-2019."}
...
```{r phillyHomeCostPerBurglaryInflation, fig.cap = "The inflation-adjusted annual number of burglaries and cost per burglary for daytime home burglaries in Philadelphia, 1960-2019."}
inflation_values <- inflation_adjust(base_year = 2019)
inflation_values$multiplier <- inflation_values$pct_increase / 100
inflation_values$multiplier <- abs(inflation_values$multiplier)
inflation_values$multiplier <- 1-inflation_values$multiplier
inflation_values$multiplier <- 1/inflation_values$multiplier
inflation_values$year       <- as.numeric(inflation_values$year)

temp <-
  temp %>%
  left_join(inflation_values, by = "year") %>%
  mutate(inflation_cost_per_burglary = cost_per_burglary * multiplier,
         inflation_value_burg_resident_day = value_burg_resident_day * multiplier)
...
ucr_property_stolen_recovered_yearly_1960_2019 %>%
...
```{r agenciesInBoth, fig.cap = "The percent of agencies in the Supplement to Return A data that are also in the Offenses Known and Clearances by Arrest (Return A) data in that year, 1960-2019."}
agencies_in_offenses <- data.frame(year = 1960:2018,
                                   percent = NA)
for (i in 1:nrow(agencies_in_offenses)) {
  year_temp <- agencies_in_offenses$year[i]
  offenses_agencies <- offenses_known_yearly_1960_2018 %>%
    dplyr::filter(year %in% year_temp,
                  !last_month_reported %in% "no months reported")
  property_agencies <- ucr_property_stolen_recovered_yearly_1960_2019 %>%
    dplyr::filter(year %in% year_temp,
                  number_of_months_reported != 0) 
  
  table(property_agencies$ori %in% offenses_agencies$ori)
  offenses_agencies <- offenses_agencies[offenses_agencies$ori %in% offenses_agencies$ori, ]
...
```{r nycLivestock, fig.cap = "The annual value of stolen livestock in New York City, 1960-2019."}
ucr_property_stolen_recovered_yearly_1960_2019 %>%
