...
Figure \@ref(fig:PhillyBurglaryCost) measures this cost of crime by showing the annual sum of the value of the property stolen for daytime home burglaries in Philadelphia. While the number of burglaries peaked in the early 1980s, the total value of burglaries didn't peak until the early 2010s, so the cost of this crime (even this very narrow measure of cost) can't be ascertained from knowing the number of burglaries alone. This data just tells us the value of the items stolen, so we can't explain 
...
We can put these variables together to look at the cost per burglary so we can roughly measure the cost for each burglary victim. Figure \@ref(fig:phillyHomeCostPerBurglary) shows the cost average cost per burglary for each year of data available. Now we have a different story than the other graphs. Even though the number of daytime home burglaries declined over the last decade and the total , the 
...
```{r phillyGunsValue, fig.cap ="The annual value of stolen guns in Philadelphia, PA, 1960-2018"}
ucr_property_stolen_recovered_yearly_1960_2019 %>%
  dplyr::filter(ori %in% "PAPEP00") %>%
  filter(year < 2019) %>%
